...
/* ---- PLLI2S: I2S2/I2S3 的时钟源 ----------------------------------------
 * I2SxCLK = ((HSE_VALUE / PLL_M) * PLLI2S_N) / PLLI2S_R
 *         = (8MHz / 8) * 271 / 2 = 135.5 MHz
 *
 * 为什么不用常见的 258/3(=86MHz)：44.1kHz 的分频精度差很多
 *   135.5MHz -> I2SDIV=6 -> 实际 44108 Hz，误差 0.02%（听不出来）
 *    86.0MHz -> I2SDIV=4 -> 实际 41992 Hz，误差 4.8%（音调明显偏低）
 * ---------------------------------------------------------------------- */
...
#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F401xx)
    /* ========================================================================
     * 配置并启用 PLLI2S —— I2S2/I2S3 唯一的时钟源
     *
     * ★ ST 的模板里 PLLI2S_N/PLLI2S_R 是注释掉的 NA，RCC->PLLI2SCFGR
     *   从来没被写过。不启用它 I2S 就没有时钟，音频一点声音都不会有，
     *   而且不会报任何错 —— 只是安静地没声音。
     * ===================================================================== */
