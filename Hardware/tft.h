...
#define GREEN         	 0x07E0
#define CYAN          	 0x07FF   /* 原来写的是 0x7FFF，和 lcd_ili9488.h 冲突且颜色不对 */
...
/* ★ 分块(流式)显示用：只往【当前已设好的窗口】里推像素，绝不重设窗口。
 * 靠 ILI9488 的地址计数器自己往下走，写满窗口后自动回卷到 (XS,YS)。 */
void TFT_PushPixels(const uint16_t *pixels, uint32_t count);
void Test_Red_Block(void);
